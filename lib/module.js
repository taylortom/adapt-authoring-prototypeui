...
const { AbstractModule, Requester, Responder } = require('adapt-authoring-core');
/**
* Provides a UI for the prototype functionality
* @extends {AbstractModule}
*/
class PrototypeUIModule extends AbstractModule {
  constructor(app, pkg) {
    super(app, pkg);
    this.apiUrl = app.config.get('adapt-authoring-server.url');
  }
  /**
  * Initialises the API
  ...
  */
  handleUsers(req, res, next) {
    let users, error;
    new Requester(`${this.apiUrl}/api/users`).get()
...
module.exports = PrototypeUIModule;
